...
@objc(BarkoderFlutterPlugin)
public class BarkoderFlutterPlugin: NSObject, FlutterPlugin {
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        ...
        registrar.register(viewFactory, withId: BarkoderFlutterPlugin.VIEW_ID)
    }
    ...
    private func setMethodCallHandler() {
        channel.setMethodCallHandler { [weak self] call, result in
            ...
            case "setRoiCenterMark":
                self?.setRoiCenterMark(call, result: result)
            ...
            case "setMatchFilter":
                self?.setMatchFilter(call, result: result)
            case "setReturnOnlyMatchedResults":
                self?.setReturnOnlyMatchedResults(call, result: result)
            ...
            case "getMatchFilter":
                self?.getMatchFilter(result)
            case "getReturnOnlyMatchedResults":
                self?.getReturnOnlyMatchedResults(result)
            ...
            case "setARReturnOnlyMatchedResults":
                self?.setARReturnOnlyMatchedResults(call, result: result)
            case "setARDisplayOnlyMatchedResults":
                self?.setARDisplayOnlyMatchedResults(call, result: result)
            ...
            case "getARReturnOnlyMatchedResults":
                self?.getARReturnOnlyMatchedResults(result)
            case "getARDisplayOnlyMatchedResults":
                self?.getARDisplayOnlyMatchedResults(result)
...
extension BarkoderPlatformView {
    ...
    private func setRoiCenterMark(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        guard let index = call.arguments as? Int else {
            return
        }
    ...
    private func setMatchFilter(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        guard let matchFilter = call.arguments as? String else { return }

        barkoderView.config?.decoderConfig?.matchFilter = matchFilter

        result(nil)
    }

    private func setReturnOnlyMatchedResults(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    private func setARReturnOnlyMatchedResults(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        guard let value = call.arguments as? Bool else { return }
        barkoderView.config?.arConfig.returnOnlyMatchedResults = value
        result(nil)
    }

    private func setARDisplayOnlyMatchedResults(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        guard let value = call.arguments as? Bool else { return }
    ...
    private func resetARCache(_ result: @escaping FlutterResult) {
...
extension BarkoderPlatformView {
    ...
    private func getMatchFilter(_ result: @escaping FlutterResult) {
        result(barkoderView.config?.decoderConfig?.matchFilter)
    }

    private func getReturnOnlyMatchedResults(_ result: @escaping FlutterResult) {
        result(barkoderView.config?.decoderConfig?.returnOnlyMatchedResults)
    }
    ...
    private func getRoiCenterMark(_ result: @escaping FlutterResult) {
        result(barkoderView.config?.roiCenterMark.rawValue)
    }
    ...
    private func getARReturnOnlyMatchedResults(_ result: @escaping FlutterResult) {
        result(barkoderView.config?.arConfig.returnOnlyMatchedResults)
    }

    private func getARDisplayOnlyMatchedResults(_ result: @escaping FlutterResult) {
        result(barkoderView.config?.arConfig.displayOnlyMatchedResults)
    }
    ...
    private func getDeviceId(_ result: @escaping FlutterResult) {
        result(barkoderView.config?.decoderConfig?.getDeviceId())
    }
}
